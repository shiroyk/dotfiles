...
NETEASE_CMD = """dbus-send --print-reply --dest=org.mpris.MediaPlayer2.netease-cloud-music
                 /org/mpris/MediaPlayer2 org.mpris.MediaPlayer2.Player.{cmd}"""
...
class Py3status:
    ...
    def _netease_cmd(self, action):
        return NETEASE_CMD.format(cmd=action)
    ...
    def _get_text(self):
        ...
        try:
            self.__bus = bus.get_object(
                "org.mpris.MediaPlayer2.netease-cloud-music", "/org/mpris/MediaPlayer2"
            )
    ...
    def on_click(self, event):
        ...
        if button == self.button_play_pause:
            self.py3.command_run(self._netease_cmd("PlayPause"))
            sleep(0.1)
        elif button == self.button_next:
            self.py3.command_run(self._netease_cmd("Next"))
            sleep(0.1)
        elif button == self.button_previous:
            self.py3.command_run(self._netease_cmd("Previous"))
            sleep(0.1)
